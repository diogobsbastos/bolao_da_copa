...
export async function rotasJogar(app: FastifyInstance) {
  ...
  app.get("/jogar/dados", async (req, reply) => {
    ...
    let ultRod = { rodada: 0, pts: 0, dia: "" };
    try { const rr = (await pool.query("SELECT j.rodada r, COALESCE(sum(pb.pontos),0) p, to_char(max(j.inicio AT TIME ZONE 'America/Sao_Paulo'),'DD/MM') dia FROM palpites_bolao pb JOIN jogos j ON j.id=pb.jogo_id WHERE pb.usuario_id=$1 AND pb.creditado=true GROUP BY j.rodada ORDER BY j.rodada DESC LIMIT 1", [u.id])).rows[0] as any; if (rr) { ultRod.rodada = Number(rr.r || 0); ultRod.pts = Number(rr.p || 0); ultRod.dia = String(rr.dia || ""); } } catch {}
