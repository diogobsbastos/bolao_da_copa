...
export async function rotasJogar(app: FastifyInstance) {
  ...
  app.get("/jogar/dados", async (req, reply) => {
    ...
    let longoFeito = false; try { longoFeito = !!((await pool.query("SELECT 1 FROM palpites_longo WHERE usuario_id=$1 AND campeao IS NOT NULL AND campeao<>''", [u.id])).rowCount); } catch {}
    return { ok: true, autoPreencher: autoP, iaConectada: iaOn, acessoFull, me: { id: u.id, nome: u.nome || u.email, email: u.email, papel: u.papel, nomeTime: pf.nome_time || "", avatar: pf.avatar || "", pagou: !!pf.pagou, valorPago }, carteiras: { saldo: Number(cart.saldo || 0) }, ranking: { pos, pontos, total }, proximo, palpitesPendentes: pend, mataAberto, longoFeito };
  });
