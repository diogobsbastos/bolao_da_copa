...
export async function rotasJogar(app: FastifyInstance) {
  ...
  app.get("/jogar/dados", async (req, reply) => {
    ...
    let poteTot=0;try{const _pr=await pool.query("SELECT COALESCE(SUM(valor),0) AS v FROM depositos WHERE status='approved'");poteTot=Number(_pr.rows[0].v)||0;}catch{} try{const _sp=await pool.query("SELECT COALESCE(SUM(valor),0) AS v FROM patrocinadores WHERE status='ativo'");poteTot+=Number(_sp.rows[0].v)||0;}catch{}
    ...
    try {
      const rows = (await pool.query(`
        ...
        SELECT a.nome, a.selecao, a.gols,
               (SELECT j2.figurinha FROM jogadores j2
                 WHERE j2.figurinha IS NOT NULL
                   AND (lower(j2.nome)=lower(a.nome) OR lower(a.nome) LIKE '%'||lower(j2.nome)||'%' OR lower(j2.nome) LIKE '%'||lower(a.nome)||'%')
                 ORDER BY (lower(j2.nome)=lower(a.nome)) DESC, length(j2.nome) DESC LIMIT 1) AS figurinha
          FROM agg a
         ORDER BY a.gols DESC, a.nome
  ...
  app.get("/jogar/copa", async (req, reply) => {
    ...
    const proxRows = (await pool.query("SELECT selecao_casa, selecao_visitante, inicio, rodada FROM jogos WHERE inicio>=now() AND selecao_casa<>'A definir' AND selecao_visitante<>'A definir' ORDER BY inicio ASC LIMIT 20")).rows as any[];
  ...
  app.get("/jogar/news", async (req, reply) => {
    ...
    return { ok:true, itens: nf.slice(0,15) };
  });
