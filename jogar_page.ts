  ...
  <section class="sec" id="s-ia">
   ...
   <div class="card">
     ...
     <div id="step-key" style="display:none">
      ...
      <div class="iarow rgt"><span id="ia-st" class="muted" style="margin-right:auto"></span><button class="btn ghost" id="btn-test" onclick="testarIA()" disabled>&#129514; Testar</button><button class="btn" id="btn-conn" onclick="conectarIA()" disabled>&#128279; Conectar</button></div>
     </div>
...
function iaKeyCheck(){var pv=document.getElementById("ia-prov").value;var k=(document.getElementById("ia-key").value||"").trim();var min=IAMIN[pv]!=null?IAMIN[pv]:20;var ok=k.length>=min;var bt=document.getElementById("btn-test"),bc=document.getElementById("btn-conn");if(bt)bt.disabled=!ok;if(bc)bc.disabled=!ok;var st=document.getElementById("ia-st");if(st&&st.style.color===""){if(!k.length){st.textContent="";}else if(!ok){st.textContent="faltam "+(min-k.length)+" caracteres";}else{st.textContent="chave ok ✔";}}}
