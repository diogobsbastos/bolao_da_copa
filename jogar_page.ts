...
async function desconectarIA(){if(!confirm("Desconectar sua IA? Sua chave ser\u00e1 removida e voc\u00ea pode reconectar quando quiser."))return;var r=await fetch(BASE+"/jogar/ia/desconectar",{method:"POST",headers:H()});var d=await r.json().catch(function(){return{};});if(d&&d.ok){toast("IA desconectada");var k=document.getElementById("ia-key");if(k)k.value="";loadIA();}else{toast((d&&d.erro)||"erro",1);}}
