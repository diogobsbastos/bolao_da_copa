...
async function desconectarIAok(){fecha();var r=await fetch(BASE+"/jogar/ia/desconectar",{method:"POST",headers:H()});var d=await r.json().catch(function(){return{};});if(d&&d.ok){toast("IA desconectada");var k=document.getElementById("ia-key");if(k)k.value="";loadIA();}else{toast((d&&d.erro)||"erro",1);}}
