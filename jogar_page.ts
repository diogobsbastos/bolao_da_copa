...
function iaKeyCheck(){var pv=document.getElementById("ia-prov").value;var k=(document.getElementById("ia-key").value||"").trim();var min=IAMIN[pv]!=null?IAMIN[pv]:20;var ok=k.length>=min;var bc=document.getElementById("btn-conn");if(bc)bc.disabled=!ok;var st=document.getElementById("ia-st");if(st&&st.style.color===""){if(!k.length){st.textContent="";}else if(!ok){st.textContent="faltam "+(min-k.length)+" caracteres";}else{st.textContent="chave ok ✔";}}}
function selProv(p){var sel=document.getElementById("ia-prov");if(sel)sel.value=p;document.querySelectorAll(".provcard").forEach(function(c){c.classList.toggle("sel",c.getAttribute("data-p")===p);});iaProvCh();var df=document.getElementById("btn-disc-flow");if(df)df.style.display="none";var sk=document.getElementById("step-key");if(sk)sk.style.display="";var sm=document.getElementById("step-model");if(sm)sm.style.display="none";var rc=document.getElementById("ia-recs");if(rc){rc.style.display="none";rc.innerHTML="";}iaKeyCheck();}
