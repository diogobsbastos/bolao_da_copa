...
async function loadIA(){var r=await fetch(BASE+"/jogar/ia",{headers:H()});var d=await r.json().catch(function(){return{};});if(!d||!d.ok)return;IACONN=!!d.conectada;var ig=document.getElementById("ia-gasto");if(ig)ig.textContent="R$ "+(Number(d.gastoBrl||0)).toFixed(2).replace(".",",");var cw=document.getElementById("ia-conectado");if(cw)cw.style.display=d.conectada?"":"none";var rs=document.getElementById("ia-resumo"),fl=document.getElementById("ia-flow");if(d.conectada){if(d.base_url)document.getElementById("ia-base").value=d.base_url;var mi=document.getElementById("ia-modelo");if(mi)mi.value=d.modelo||"";document.getElementById("ia-prov").value=d.provedor||"gemini";if(rs){rs.style.display="";rs.innerHTML='<span class="iadot">&#9989;</span><div class="iainfo"><b>IA conectada</b><small>'+esc(PNOME[d.provedor]||d.provedor||"")+' &middot; '+esc(d.modelo||"")+'</small></div><div class="iaracts"><button class="btn ghost mini" onclick="trocarIA()">&#128260; Trocar IA</button><button class="btn dang mini" onclick="desconectarIA()">&#128683; Desconectar</button></div>';}if(fl)fl.style.display="none";}else{if(rs)rs.style.display="none";if(fl)fl.style.display="";}}
function trocarIA(){var rs=document.getElementById("ia-resumo"),fl=document.getElementById("ia-flow");if(rs)rs.style.display="none";if(fl)fl.style.display="";var prov=document.getElementById("ia-prov").value||"gemini";var mod=document.getElementById("ia-modelo").value||"";selProv(prov);var k=document.getElementById("ia-key");if(k){k.value="";k.placeholder="\u2713 chave j\u00e1 conectada \u2014 cole outra s\u00f3 se quiser trocar a chave";}var st=document.getElementById("ia-st");if(st){st.textContent="\u2705 j\u00e1 conectado \u2014 s\u00f3 troque o modelo e salve (ou cole outra chave)";st.style.color="#1faa59";}var sm=document.getElementById("step-model");if(sm)sm.style.display="";renderRecs();if(mod){var mi=document.getElementById("ia-modelo");if(mi)mi.value=mod;document.querySelectorAll("#ia-recs .rec").forEach(function(c){var b=c.querySelector("b");var nm=b?b.textContent.replace("\u2b50","").trim():"";c.classList.toggle("sel",nm===mod);});}}
...
function iaKeyCheck(){var pv=document.getElementById("ia-prov").value;var k=(document.getElementById("ia-key").value||"").trim();var min=IAMIN[pv]!=null?IAMIN[pv]:20;var ok=k.length>=min;var bc=document.getElementById("btn-conn"),df=document.getElementById("btn-disc-flow");if(IACONN&&!k.length){if(df)df.style.display="";if(bc)bc.style.display="none";}else{if(df)df.style.display="none";if(bc){bc.style.display="";bc.disabled=!ok;}}var st=document.getElementById("ia-st");if(st&&st.style.color===""){if(!k.length){st.textContent="";}else if(!ok){st.textContent="faltam "+(min-k.length)+" caracteres";}else{st.textContent="chave ok ✔";}}}
function selProv(p){var sel=document.getElementById("ia-prov");if(sel)sel.value=p;document.querySelectorAll(".provcard").forEach(function(c){c.classList.toggle("sel",c.getAttribute("data-p")===p);});iaProvCh();var sk=document.getElementById("step-key");if(sk)sk.style.display="";var sm=document.getElementById("step-model");if(sm)sm.style.display="none";var rc=document.getElementById("ia-recs");if(rc){rc.style.display="none";rc.innerHTML="";}iaKeyCheck();}
