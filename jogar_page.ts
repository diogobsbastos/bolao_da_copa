 ...
 <main class="main">
  ...
  <section class="sec" id="s-ia">
   ...
   <div class="card">
     ...
     <div class="provgrid">
      <div class="provcard sel" data-p="gemini" onclick="selProv('gemini')"><b>&#10024; Google Gemini</b><small>Gr&aacute;tis (free tier) &middot; recomendado</small></div>
      <div class="provcard" data-p="anthropic" onclick="selProv('anthropic')"><b>&#129504; Anthropic (Claude)</b><small>chave pr&oacute;pria</small></div>
      <div class="provcard" data-p="openai" onclick="selProv('openai')"><b>&#129302; OpenAI</b><small>ou compat&iacute;vel</small></div>
      <div class="provcard" data-p="ollama" onclick="selProv('ollama')"><b>&#128187; Ollama</b><small>local / self-host</small></div>
     </div>
     ...
     <div class="iarow"><input id="ia-key" type="password" placeholder="cole a chave da sua LLM" style="flex:1;min-width:200px"><button class="btn ghost" id="btn-gemini-help" onclick="ajudaGemini()">&#10024; Criar Gemini gr&aacute;tis</button></div>
     <div class="iarow"><button class="btn" onclick="conectarIA()">&#128279; Conectar</button><span id="ia-st" class="muted"></span></div>
     <div id="step-model" style="display:none">
      <label class="ialbl">3. Escolha o modelo</label>
      <input id="ia-modelo" list="ia-mods" placeholder="ex.: gemini-2.5-flash"><datalist id="ia-mods"></datalist><div id="ia-recs" class="iarecs"></div>
      <div class="iarow" id="ia-actions"><button class="btn ghost" onclick="testarIA()">&#129514; Testar</button><button class="btn" onclick="salvarIA()">Salvar conex&atilde;o</button><span id="ia-st2" class="muted"></span></div>
     </div>
   </div>
...
function iaProvCh(){var p=document.getElementById("ia-prov").value;var need=(p==="openai"||p==="ollama");document.getElementById("ia-base-wrap").style.display=need?"block":"none";var bi=document.getElementById("ia-base");if(p==="ollama"&&bi&&!bi.value)bi.placeholder="http://SEU-HOST:11434/v1";var gh=document.getElementById("btn-gemini-help");if(gh)gh.style.display=(p==="gemini")?"":"none";var rc=document.getElementById("ia-recs");if(rc){rc.style.display="none";rc.innerHTML="";}var sm=document.getElementById("step-model");if(sm)sm.style.display="none";}
async function loadIA(){var r=await fetch(BASE+"/jogar/ia",{headers:H()});var d=await r.json().catch(function(){return{};});if(!d||!d.ok)return;selProv(d.provedor||"gemini");document.getElementById("ia-modelo").value=d.modelo||"";if(d.base_url)document.getElementById("ia-base").value=d.base_url;document.getElementById("ia-gasto").textContent="R$ "+(Number(d.gastoBrl||0)).toFixed(2).replace(".",",");document.getElementById("ia-st").textContent=d.conectada?"✅ conectada":"não conectada";var sm=document.getElementById("step-model");if(d.conectada){if(sm)sm.style.display="";renderRecs();}else{if(sm)sm.style.display="none";}}
...
async function testarIA(){var st=document.getElementById("ia-st2");if(st){st.style.color="";st.textContent="testando...";}var body={provedor:document.getElementById("ia-prov").value,api_key:document.getElementById("ia-key").value,base_url:document.getElementById("ia-base").value,modelo:document.getElementById("ia-modelo").value};if(!body.api_key||!body.modelo){toast("Falta key ou modelo",1);if(st)st.textContent="";return;}var r=await fetch(BASE+"/jogar/ia/testar",{method:"POST",headers:H(),body:JSON.stringify(body)});var d=await r.json().catch(function(){return{};});if(d&&d.ok){if(st){st.textContent="\u2705 funcionou! resposta: "+(d.resposta||"OK");st.style.color="#1faa59";}toast("IA respondeu \u2705");}else{if(st)st.textContent="";toast((d&&d.erro)||"falhou no teste",1);}}
...
function selProv(p){var sel=document.getElementById("ia-prov");if(sel)sel.value=p;document.querySelectorAll(".provcard").forEach(function(c){c.classList.toggle("sel",c.getAttribute("data-p")===p);});iaProvCh();}
