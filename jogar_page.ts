 ...
 <main class="main">
  ...
  <section class="sec" id="s-ia">
   ...
   <div class="card">
    ...
    <div class="iarow"><button class="btn" onclick="salvarIA()">Salvar conex&atilde;o</button><button class="btn ghost" onclick="testarIA()">&#128268; Testar</button><button class="btn ghost" id="btn-gemini-help" onclick="ajudaGemini()">&#10024; Criar Gemini gr&aacute;tis</button><span id="ia-st" class="muted"></span></div>
   </div>
...
async function testarIA(){var st=document.getElementById("ia-st");st.style.color="";var key=document.getElementById("ia-key").value;if(!key){toast("Cole sua API key primeiro",1);return;}st.textContent="testando...";var body={provedor:document.getElementById("ia-prov").value,api_key:key,base_url:document.getElementById("ia-base").value};var r=await fetch(BASE+"/jogar/ia/modelos",{method:"POST",headers:H(),body:JSON.stringify(body)});var d=await r.json().catch(function(){return{};});if(d&&d.ok){var dl=document.getElementById("ia-mods");dl.innerHTML=(d.modelos||[]).map(function(m){return '<option value="'+esc(m)+'">';}).join("");st.textContent="\u2705 chave v\u00e1lida \u2014 escolha o modelo abaixo";st.style.color="#1faa59";renderRecs();}else{st.textContent="";var rc=document.getElementById("ia-recs");if(rc){rc.style.display="none";rc.innerHTML="";}toast((d&&d.erro)||"chave inv\u00e1lida",1);}}
