...
export async function rotasNotificacoes(app: FastifyInstance) {
  ...
  app.get("/admin/email/preview", async (req, reply) => {
    if (!(await admOk(req))) return reply.code(401).send({ erro: "nao autorizado" });
    const q = (req.query || {}) as any;
    const base = (await getCfg("base_url_publica")) || "https://oracle-vipworks.duckdns.org/bolao-copa26";
    let html = "";
    if (q.ag) {
      const ag = (await pool.query("SELECT * FROM notif_agendamentos WHERE id=$1", [Number(q.ag)])).rows[0] as any;
      if (!ag) return reply.code(404).type("text/html").send("<p style='font-family:sans-serif;padding:24px'>Agendamento nao encontrado.</p>");
      if (ag.tipo === "lancamento") { html = (await montarLancamento()).html; }
      else { html = htmlEmail(ag.titulo || "Bolão Copa 26", ag.texto || "", "Ir pro Bolão", base + "/"); }
    } else if (q.tipo === "lancamento") {
      html = (await montarLancamento()).html;
    } else {
      html = htmlEmail(String(q.titulo || "Título da mensagem"), String(q.texto || "Texto da mensagem"), "Ir pro Bolão", base + "/");
    }
...
function verEmail(id){var w=window.open("","_blank");if(!w){toast("Permita pop-ups pra ver o preview",1);return;}fetch(nb()+"/admin/email/preview?ag="+id,{headers:H()}).then(function(r){return r.text();}).then(function(html){try{w.document.open();w.document.write(html);w.document.close();}catch(e2){}}).catch(function(){});}
