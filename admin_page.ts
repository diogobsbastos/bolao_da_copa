...
async function loadUsers(){
 ...
 if(!r.ok){b.textContent="conecte para ver.";return;}
 var rows=await r.json();UEMAIL={};
 var h='<table><thead><tr><th>ID</th><th>E-mail</th><th>Nome</th><th>Papel</th><th>A&ccedil;&atilde;o</th></tr></thead><tbody>';
 for(var i=0;i<rows.length;i++){var u=rows[i];UEMAIL[u.id]=u.email||"";
  h+='<tr><td>'+u.id+'</td><td>'+esc(u.email)+'</td><td>'+esc(u.nome||"")+'</td><td>'+esc(u.papel||"jogador")+'</td><td><button onclick="resetSenha('+u.id+')" style="padding:4px 9px;font-size:12px;border:0;border-radius:7px;background:#4361ee;color:#fff;font-weight:700;cursor:pointer">&#128273; Redefinir senha</button></td></tr>';
 }
 h+='</tbody></table>';b.innerHTML=h;
}
async function resetSenha(id){
 var email=UEMAIL[id]||("#"+id);
 var nova=prompt("Nova senha para "+email+" (deixe vazio para gerar uma tempor\u00e1ria):","");
 if(nova===null)return;
 ...
 alert("Senha de "+j.email+" redefinida para:  "+j.senha+"   (passe para a pessoa; ela pode trocar depois).");
}
