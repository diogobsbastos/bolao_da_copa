...
var _noticiasLista=[];
async function loadNoticiasLLM(){var r=await fetch(BASE+"/admin/llm/noticias",{headers:H()});if(!r.ok)return;var d=await r.json();_noticiasLista=d.lista||[];var ids=d.ids||[];[1,2,3].forEach(function(n){var s=document.getElementById("sel-noticias-"+n);if(s){s.innerHTML='<option value="">&mdash; nenhuma &mdash;</option>';s.value=ids[n-1]||"";}});renderNoticiasSelects();}
function renderNoticiasSelects(){var raw=[1,2,3].map(function(n){var s=document.getElementById("sel-noticias-"+n);return s?s.value:"";});var seen={};var cur=raw.map(function(v){if(v&&seen[v])return "";if(v)seen[v]=1;return v;});[1,2,3].forEach(function(n){var s=document.getElementById("sel-noticias-"+n);if(!s)return;var mine=cur[n-1];var taken=cur.filter(function(v,i){return i!==(n-1)&&v;});var opts='<option value="">&mdash; nenhuma &mdash;</option>';_noticiasLista.forEach(function(p){var id=String(p.id);if(taken.indexOf(id)>=0)return;opts+='<option value="'+id+'"'+(id===mine?" selected":"")+'>'+p.provedor+' / '+p.modelo+'</option>';});s.innerHTML=opts;s.value=mine;});}
