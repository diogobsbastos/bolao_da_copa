...
function nvNovo(){window._nvId=null;var f=document.getElementById("nv-form");if(f)f.classList.remove("hide");var ti=document.getElementById("nv-ti");if(ti)ti.textContent="Novo motor NVIDIA";["nv-key","nv-modelo","nv-prompt"].forEach(function(i){var e=document.getElementById(i);if(e)e.value="";});var ks=document.getElementById("nv-keyset");if(ks)ks.textContent="";var rs=document.getElementById("nv-result");if(rs)rs.innerHTML="";var ms=document.getElementById("nv-msg");if(ms)ms.textContent="";}
function nvFechar(){var f=document.getElementById("nv-form");if(f)f.classList.add("hide");}
function nvEditar(id){var arr=window._nvAll||[];var nv=null;for(var i=0;i<arr.length;i++){if(arr[i].id===id){nv=arr[i];break;}}if(!nv)return;window._nvId=id;var f=document.getElementById("nv-form");if(f)f.classList.remove("hide");var ti=document.getElementById("nv-ti");if(ti)ti.textContent="Editar: "+nv.modelo;var mo=document.getElementById("nv-modelo");if(mo)mo.value=nv.modelo||"";var k=document.getElementById("nv-key");if(k)k.value="";var ks=document.getElementById("nv-keyset");if(ks)ks.textContent=nv.api_key_set?"— chave salva ✓ (em branco mantem)":"— SEM CHAVE, cole a nvapi-";var rs=document.getElementById("nv-result");if(rs)rs.innerHTML="";var ms=document.getElementById("nv-msg");if(ms)ms.textContent="";}
...
async function testarNvId(id){toast("Gerando (cold start ~1min)...","ok");var r=await fetch(BASE+"/admin/criador-fig/testar-imagem",{method:"POST",headers:H(),body:JSON.stringify({id:id})});var j=await r.json().catch(function(){return{};});var rs=document.getElementById("nv-result");var f=document.getElementById("nv-form");if(f)f.classList.remove("hide");if(!rs)return;if(!j.ok){rs.innerHTML='<div style="color:#e23744;font-weight:600">FALHOU: '+esc(j.erro||"erro")+'</div>';return;}rs.innerHTML='<img src="'+j.img+'" style="max-width:340px;border-radius:10px;border:1px solid var(--bd)"><div class="sub" style="margin-top:4px">motor: '+esc((j.motor&&j.motor.modelo)||"")+'</div>';}
