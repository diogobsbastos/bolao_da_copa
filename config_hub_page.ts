   ...
   <section id="pg-img" class="hide">
    ...
    <div id="img-nvidia" class="hide">
     <div class="card" style="border-left:4px solid #76b900">
      <div class="cardhead"><div><h3>&#127918; NVIDIA FLUX &mdash; motores</h3>
       <div class="sub">Cadastre quantos modelos quiser (dev, kontext, schnell...). Todos prontos p/ consumir. Endpoint fixo: <code>https://ai.api.nvidia.com/v1/genai/&lt;modelo&gt;</code> &middot; chave <code>nvapi-</code> (migravel p/ outros projetos).</div></div>
       <button class="add" onclick="nvNovo()">&#10133; Inserir novo</button></div>
      <div id="nv-list"><div class="muted">carregando...</div></div>
      <div id="nv-testout" style="margin-top:10px"></div>
      <div id="nv-form" class="card hide" style="margin-top:12px;background:rgba(118,185,0,.05)">
       <div class="cardhead"><h3 id="nv-ti" style="font-size:15px">Novo motor NVIDIA</h3><button class="sm gh" onclick="nvFechar()">fechar</button></div>
       ...
       <div class="sub" style="margin-top:6px"><b>flux.1-dev</b>: texto&rarr;imagem. <b>flux.1-kontext-dev</b>: edita a partir de foto (precisa de imagem; usado no Gerar nano/avatar).</div>
       <div class="save" style="margin-top:12px">
        <button class="sm" onclick="salvarNvidia()">Salvar</button>
        <button class="sm gh" onclick="testarImgNvidia()">&#9889; Testar (so flux.1-dev)</button>
...
function imgTab(w){var llm=w==="llm";document.getElementById("img-llm").classList.toggle("hide",!llm);document.getElementById("img-nvidia").classList.toggle("hide",llm);document.getElementById("imgtab-llm").className=llm?"sm":"sm gh";document.getElementById("imgtab-nv").className=llm?"sm gh":"sm";if(!llm)loadNvList();}
async function loadNvList(){var box=document.getElementById("nv-list");if(!box)return;var r=await fetch(BASE+"/admin/llm?papel=imagem",{headers:H()});if(!r.ok){box.innerHTML='<div class="muted">erro</div>';return;}var lista=await r.json();var nv=lista.filter(function(m){return String(m.base_url||"").toLowerCase().indexOf("nvidia")>=0;});window._nvAll=nv;if(!nv.length){box.innerHTML='<div class="muted">nenhum motor NVIDIA ainda. Clique em <b>Inserir novo</b>.</div>';return;}box.innerHTML=nv.map(function(m){return '<div class="prov'+(m.em_uso?" uso":"")+'">'+(m.em_uso?'<span class="pill uso">EM USO</span>':'')+'<div class="pinfo"><b>'+esc(m.modelo)+'</b> '+(m.api_key_set?'<span class="muted">chave ✓</span>':'<span style="color:#e23744;font-weight:700">SEM CHAVE</span>')+'</div><div class="pacts">'+(m.em_uso?'':'<button class="sm gr" onclick="usarNvId('+m.id+')">usar</button>')+'<button class="sm gh" onclick="testarNvId('+m.id+')">testar</button><button class="sm gh" onclick="nvEditar('+m.id+')">editar</button><button class="sm dg" onclick="delNv('+m.id+')">x</button></div></div>';}).join("");}
function nvNovo(){window._nvId=null;var f=document.getElementById("nv-form");if(f){f.classList.remove("hide");f.scrollIntoView({behavior:"smooth",block:"center"});}var ti=document.getElementById("nv-ti");if(ti)ti.textContent="Novo motor NVIDIA";["nv-key","nv-modelo","nv-prompt"].forEach(function(i){var e=document.getElementById(i);if(e)e.value="";});var ks=document.getElementById("nv-keyset");if(ks)ks.textContent="";var rs=document.getElementById("nv-result");if(rs)rs.innerHTML="";var ms=document.getElementById("nv-msg");if(ms)ms.textContent="";}
function nvFechar(){var f=document.getElementById("nv-form");if(f)f.classList.add("hide");}
function nvEditar(id){var arr=window._nvAll||[];var nv=null;for(var i=0;i<arr.length;i++){if(arr[i].id===id){nv=arr[i];break;}}if(!nv)return;window._nvId=id;var f=document.getElementById("nv-form");if(f){f.classList.remove("hide");f.scrollIntoView({behavior:"smooth",block:"center"});}var ti=document.getElementById("nv-ti");if(ti)ti.textContent="Editar: "+nv.modelo;var mo=document.getElementById("nv-modelo");if(mo)mo.value=nv.modelo||"";var k=document.getElementById("nv-key");if(k)k.value="";var ks=document.getElementById("nv-keyset");if(ks)ks.textContent=nv.api_key_set?"— chave salva ✓ (em branco mantem)":"— SEM CHAVE, cole a nvapi-";var rs=document.getElementById("nv-result");if(rs)rs.innerHTML="";var ms=document.getElementById("nv-msg");if(ms)ms.textContent="";}
async function salvarNvidia(){var modelo=val("nv-modelo").trim()||"black-forest-labs/flux.1-dev";var key=val("nv-key");var msg=document.getElementById("nv-msg");if(msg)msg.textContent="salvando...";var body={id:window._nvId||null,papel:"imagem",provedor:"openai",base_url:"https://ai.api.nvidia.com/v1/genai",modelo:modelo,api_key:key};var r=await fetch(BASE+"/admin/llm",{method:"POST",headers:H(),body:JSON.stringify(body)});var j=await r.json().catch(function(){return{};});if(!j.id){if(msg)msg.textContent="erro ao salvar";return null;}window._nvId=j.id;var k=document.getElementById("nv-key");if(k)k.value="";if(msg)msg.textContent="salvo ✓";loadNvList();loadLLM("imagem");loadFinalidades();return j.id;}
async function usarNvId(id){await fetch(BASE+"/admin/llm/usar",{method:"POST",headers:H(),body:JSON.stringify({id:id})});toast("Definido como EM USO","ok");loadNvList();loadLLM("imagem");loadFinalidades();}
async function delNv(id){if(!confirm("Remover este motor NVIDIA?"))return;await fetch(BASE+"/admin/llm/del",{method:"POST",headers:H(),body:JSON.stringify({id:id})});loadNvList();loadLLM("imagem");loadFinalidades();}
async function testarNvId(id){var rs=document.getElementById("nv-testout");if(rs)rs.innerHTML='<div class="muted">gerando (cold start pode levar ~1min)...</div>';var r=await fetch(BASE+"/admin/criador-fig/testar-imagem",{method:"POST",headers:H(),body:JSON.stringify({id:id})});var j=await r.json().catch(function(){return{};});if(!rs)return;if(!j.ok){rs.innerHTML='<div style="color:#e23744;font-weight:600">FALHOU: '+esc(j.erro||"erro")+'</div>';return;}rs.innerHTML='<img src="'+j.img+'" style="max-width:340px;border-radius:10px;border:1px solid var(--bd)"><div class="sub" style="margin-top:4px">motor: '+esc((j.motor&&j.motor.modelo)||"")+'</div>';}
