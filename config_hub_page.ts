...
<div class="card"><div class="cardhead"><div><h3>&#128240; Noticias das selecoes &mdash; NewsData.io <span class="pill local">a configurar</span></h3>
   ...
   <section id="pg-llms" class="hide">
    ...
    <div class="card" style="margin-top:14px"><div class="cardhead"><div><h3>&#128240; Leitor de Not&iacute;cias &mdash; LLM dedicada</h3>
     ...
     <div style="margin-top:8px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px"><div><label class="muted" style="font-size:12px">1&ordf; op&ccedil;&atilde;o</label><select id="sel-noticias-1" onchange="salvarNoticiasLLM()" style="width:100%;padding:9px;border:1px solid var(--bd);border-radius:9px"></select></div><div><label class="muted" style="font-size:12px">2&ordf; (fallback)</label><select id="sel-noticias-2" onchange="salvarNoticiasLLM()" style="width:100%;padding:9px;border:1px solid var(--bd);border-radius:9px"></select></div><div><label class="muted" style="font-size:12px">3&ordf; (fallback)</label><select id="sel-noticias-3" onchange="salvarNoticiasLLM()" style="width:100%;padding:9px;border:1px solid var(--bd);border-radius:9px"></select></div></div><div style="margin-top:6px"><span id="msg-noticias" class="muted"></span> <span class="sub">Tenta a 1&ordf;; se falhar/estourar limite, espera e tenta 2&ordf;, depois 3&ordf; (e volta pra 1&ordf;).</span></div>
...
async function loadNoticiasLLM(){var r=await fetch(BASE+"/admin/llm/noticias",{headers:H()});if(!r.ok)return;var d=await r.json();var ids=d.ids||[];var base='<option value="">&mdash; nenhuma &mdash;</option>'+(d.lista||[]).map(function(p){return '<option value="'+p.id+'">'+p.provedor+' / '+p.modelo+'</option>';}).join("");[1,2,3].forEach(function(n){var s=document.getElementById("sel-noticias-"+n);if(s){s.innerHTML=base;s.value=ids[n-1]||"";}});}
async function salvarNoticiasLLM(){var ids=[1,2,3].map(function(n){var s=document.getElementById("sel-noticias-"+n);return s?s.value:"";});var m=document.getElementById("msg-noticias");if(m)m.textContent="salvando...";var r=await fetch(BASE+"/admin/llm/noticias",{method:"POST",headers:H(),body:JSON.stringify({ids:ids})});if(m)m.textContent=r.ok?"salvo \u2713":"erro";}
